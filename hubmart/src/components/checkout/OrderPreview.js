...
function OrderPreview({ previousPage, cartItems,placeOrder, handleSubmit }) {
  ...
  return (
    <div>
      {cartItems.map((cart) => {
        return (
          <form key={cart.id} onSubmit={handleSubmit(onSubmit)} >
            ...
            <ul className="order-details1">
              <li className="order-left-details">
                <Field type="text" name="order" value={cart.name} readOnly />X{" "}
                <Field type="text" name="quantity" className="order-right-details1" value={cart.quantity} readOnly /> <br />
...
const orderPreview = connect(mapStateToProps,{placeOrder})(OrderPreview);
